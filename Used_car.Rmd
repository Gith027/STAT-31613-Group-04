...
hist(df$mileage_kmpl,main = "Histrogram for Mileage", xlab = "Mileage",col="pink")
hist(df$engine_cc,main = "Histrogram for Engine CC", xlab = "Engine CC",col="blue")
hist(df$price_usd,main = "Histrogram for Used car prices", xlab = "Price",col="green")
hist(df$accidents_reported,main = "Histrogram for Reported accidents", xlab = "Reported accident",col="yellow")
hist(df$owner_count,main = "Histrogram for Owner counts", xlab = "owner counts",col="pink")
hist(df$make_year,main = "Histrogram for car make_year", xlab = "make_year",col="brown")
...
```{r}
#Response variable is right skewed so there is not satisfied normality assumption 
#Now we use log transformation for price_usd
df$sqrt_price_usd <- sqrt(df$price_usd)
hist(df$sqrt_price_usd,main = "Histrogram for transformed Used car prices", xlab = "Sqrt_Price",col="green")
```


```{r}
#remove the price_usd 
new_df <- df[,-6]
head(new_df)
```
...
model <- lm(sqrt_price_usd ~ ., data = new_df)
summary(model)
```
...
model1 <- lm(sqrt_price_usd ~ make_year+mileage_kmpl+engine_cc+fuel_type+owner_count+color+brand, data = new_df)
summary(model1)
```
